...
public enum Messages {
    ...
    WELCOME_TO_THE_CHATBOT("Telegram Bot: Добро пожаловать в чат-бот, выполняющий тяжелейшую работу посредника в " +
            "переговорах между Вами и OpenAI ChatGPT.\nМожно просто написать сообщение и оно будет передано в " +
            "ChatGPT, после чего Бот ответит Вам.\nМожно вызвать меню командой /menu\nМожно попросить Telegram бота о помощи " +
            "командой /help"),

    NO_RESPONSE("Telegram Bot: Прошу прощения, но ответа от ChatGPT нет!"),
    ...
    SUPPORT("Telegram Bot: Связаться с поддержкой вы можете написав сообщение по адресу https://t.me/ans_dev"),
    I_CANNOT_PROCESS_PHOTOS("Telegram Bot: Вы отправили мне фото. Прикольное. Но я не знаю, что с ним делать, пришлите пожалуйста текст.");
